      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 inset-x-0 mx-auto max-w-md bg-[#a06235] border-t-4 border-[#5c3a21] rounded-t-3xl z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] max-h-[80vh] flex flex-col"
          >
            <div className="p-4 flex items-center justify-between border-b-2 border-black/10">
              ...
              <button onClick={onClose} className="bg-black/20 p-2 rounded-full hover:bg-black/30">
                <X size={24} />
              </button>
            </div>
...
/** Returns badge count for the top-bar trigger button */
export function useAchievementCount() {
  const { telegramId } = useUser();
  const { data: earnedAchievements } = useGetAchievements(telegramId);
  return earnedAchievements?.length ?? 0;
}
