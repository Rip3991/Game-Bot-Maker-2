...
export const ConveyorBelt: React.FC<ConveyorBeltProps> = ({ state }) => {
  ...
  return (
    <div className="w-16 h-full border-l-2 border-white/10 bg-gradient-to-b from-[#0d1a05] to-[#1a2e08] relative flex flex-col shadow-[-4px_0_12px_rgba(0,0,0,0.4)] z-0 overflow-hidden">
      {/* Depo (Warehouse) icon at top */}
      <div className="h-14 flex flex-col items-center justify-center bg-black/30 border-b border-white/10 flex-shrink-0 gap-0.5">
      ...
      <div className="flex-1 relative overflow-hidden">
        {/* Scrolling stripe texture — going downward */}
        ...
        {/* Moving items — top to bottom (warehouse → sales point) */}
        {items.map(item => (
          <div
            key={item.id}
      ...
      {/* Satış Noktası (Sales Point) icon at bottom */}
      <div className="h-14 flex flex-col items-center justify-center bg-black/30 border-t border-white/10 flex-shrink-0 gap-0.5">
        <span className="text-xl drop-shadow-lg">🏪</span>
        <span className="text-[8px] font-black text-green-400/80 uppercase tracking-wide">Satış</span>
      </div>
      ...
      <style>{`
        ...
        .belt-stripe {
          ...
          animation: stripeScrollDown 0.8s linear infinite;
        }
      `}</style>
    </div>
  );
